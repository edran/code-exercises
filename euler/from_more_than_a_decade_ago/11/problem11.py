...
f = open('grid.txt', 'rU') #grid.txt must stay into  the same folder as the source

# Load data into 20x20 list
for m in range(20):
    x.append([])
    for n in range(20):
        x[m].append(int(f.read(3))) #x[m][n]
...
for k in range(20):
    for i in range(17):
        result = (x[k][i])*(x[k][i+1])*(x[k][i+2])*(x[k][i+3])
        if result > best:
            best = result
            print best
#verticale da sopra
for i in range(20):
    for k in range(17):
        result = (x[k][i])*(x[k+1][i])*(x[k+2][i])*(x[k+3][i])
        if result > best:
            best = result
            print best
...
for k in range(17):
    for i in range(17):
        result = (x[k][i])*(x[k+1][i+1])*(x[k+2][i+2])*(x[k+3][i+3])
        if result > best:
            best = result
            print best
#diagonale dx to sx
